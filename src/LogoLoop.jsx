...
const useAnimationLoop=(trackRef,targetVelocity,seqWidth,isHovered,hoverSpeed)=>{const raf=useRef(null),last=useRef(null),offset=useRef(0),velocity=useRef(0);useEffect(()=>{const track=trackRef.current;if(!track)return;if(seqWidth>0){offset.current=((offset.current%seqWidth)+seqWidth)%seqWidth;track.style.transform=`translate3d(${-offset.current}px,0,0)`}const animate=timestamp=>{if(last.current===null)last.current=timestamp;const dt=Math.max(0,timestamp-last.current)/1000;last.current=timestamp;const target=isHovered&&hoverSpeed!==undefined?hoverSpeed:targetVelocity;velocity.current+=(target-velocity.current)*(1-Math.exp(-dt/CONFIG.SMOOTH_TAU));if(seqWidth>0){offset.current=((offset.current+velocity.current*dt)%seqWidth+seqWidth)%seqWidth;track.style.transform=`translate3d(${-offset.current}px,0,0)`}raf.current=requestAnimationFrame(animate)};raf.current=requestAnimationFrame(animate);return()=>{if(raf.current!==null)cancelAnimationFrame(raf.current);last.current=null}},[targetVelocity,seqWidth,isHovered,hoverSpeed,trackRef])}
...
const LogoLoop=memo(({logos,speed=120,direction='left',width='100%',logoHeight=28,gap=32,hoverSpeed=0,fadeOut=false,fadeOutColor,scaleOnHover=false,renderItem,ariaLabel='能力标签',className='',style})=>{
  const containerRef=useRef(null),trackRef=useRef(null),seqRef=useRef(null);const[seqWidth,setSeqWidth]=useState(0),[copies,setCopies]=useState(CONFIG.MIN_COPIES),[hovered,setHovered]=useState(false)
  ...
  useAnimationLoop(trackRef,velocity,seqWidth,hovered,hoverSpeed)
  const vars={'--logoloop-gap':`${gap}px`,'--logoloop-logoHeight':`${logoHeight}px`,...(fadeOutColor&&{'--logoloop-fadeColor':fadeOutColor})}
  const render=useCallback((item,key)=><li className="logoloop__item" key={key}>{renderItem?renderItem(item,key):item.node?<span className="logoloop__node">{item.node}</span>:<img src={item.src} alt={item.alt??''} draggable={false}/>}</li>,[renderItem])
